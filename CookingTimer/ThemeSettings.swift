...
class ThemeSettings {
    ...
    private func save() {
        ...
        let str = comps.prefix(4).map { String(format: "%.6f",$0) }.joined(separator: ",")
        UserDefaults.standard.set(str, forKey: "backgroundColorComponents")
    }
    
    private func load() {
        guard let str = UserDefaults.standard.string(forKey: "backgroundColorComponents") else { return }
        let comps = str
            .split(separator: ",")
            .compactMap(Double.init)
        if comps.count >= 3 {
            mainColor = Color(.sRGB,
                              red: comps[0],
                              green: comps[1],
                              blue: comps[2],
